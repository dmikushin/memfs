...
#define FSP_FUSE_ENV_INIT                                                      \
  {                                                                            \
    'W', MemAlloc, MemFree, fsp_fuse_daemonize, fsp_fuse_set_signal_handlers,  \
        0 /*conv_to_win_path*/, 0 /*winpid_to_pid*/, {0},                      \
  }
#else
#define FSP_FUSE_ENV_INIT                                                      \
  {                                                                            \
    'W', malloc, free, fsp_fuse_daemonize, fsp_fuse_set_signal_handlers,       \
        0 /*conv_to_win_path*/, 0 /*winpid_to_pid*/, {0},                      \
  }
#endif
...
#define FSP_FUSE_ENV_INIT                                                      \
  {                                                                            \
    'C', malloc, free, fsp_fuse_daemonize, fsp_fuse_set_signal_handlers,       \
        fsp_fuse_conv_to_win_path, fsp_fuse_winpid_to_pid, {0},                \
  }
...
#define FSP_FUSE_SIGADDSET(sig)                                                \
  if (-1 != sigaction((sig), 0, &oldsa) && oldsa.sa_handler == SIG_DFL)        \
  ...
  // memset instead of initializer to avoid GCC -Wmissing-field-initializers
  // warning
...
static inline char *fsp_fuse_conv_to_win_path(const char *path) {
  ...
  return (char *)cygwin_create_path(
      0 /*CCP_POSIX_TO_WIN_A*/ | 0x100 /*CCP_RELATIVE*/, path);
}
