...
public:
  static FuseStatus create(FuseMemfs **memfs, const std::string &mountpoint,
                           fuse_mode_t mode = 00777, fuse_uid_t uid = 0,
                           fuse_gid_t gid = 0, fuse_dev_t dev = 0) {
...
private:
  FuseMemfs(const std::string &mountpoint, fuse_mode_t mode, fuse_uid_t uid,
            fuse_gid_t gid, fuse_dev_t dev)
      :
...
private:
  ...
  static int rename(const char *oldpath, const char *newpath,
    ...
    if (oldprnt == newprnt && oldname == newname)
      return 0;
    if (newnode) {
      if (int errc = self->remove_node(
              newpath, S_IFDIR == (oldnode->stat.st_mode & S_IFMT)))
        return errc;
    }
  ...
  static int link(const char *oldpath, const char *newpath) {
    ...
    newprnt->childmap[newname] = oldnode;
    oldnode->stat.st_ctim = newprnt->stat.st_ctim = newprnt->stat.st_mtim =
        now();
    return 0;
  }
  ...
  static int read(const char *path, char *buf, size_t size, fuse_off_t off,
    ...
    if (!node)
      return -ENOENT;
    fuse_off_t endoff = (std::min)(off + static_cast<fuse_off_t>(size),
                                   static_cast<fuse_off_t>(node->data.size()));
    if (off > endoff)
      return 0;
  ...
  static int setxattr(const char *path, const char *name0, const char *value,
  ...
  static int getxattr(const char *path, const char *name0, char *value,
                      size_t size) {
    auto self = getself();
    std::lock_guard<std::mutex> lock(self->_mutex);
    auto node = self->get_node(path);
  ...
  int make_node(const char *path, fuse_mode_t mode, fuse_dev_t dev,
    ...
    node = std::make_shared<MemfsNode>(++_ino, mode, context->uid, context->gid,
                                       dev);
    if (data) {
  ...
  std::shared_ptr<MemfsNode> get_node(const char *path,
...
FuseStatus mount(FuseMemfs **memfs, const std::string &mountpoint,
                 unsigned int mode, unsigned int uid, unsigned int gid,
                 unsigned int dev) {
  FuseStatus status = FuseMemfs::create(
      memfs, mountpoint, static_cast<fuse_mode_t>(mode),
      static_cast<fuse_uid_t>(uid), static_cast<fuse_gid_t>(gid),
      static_cast<fuse_dev_t>(dev));
...
FuseStatus status(FuseMemfs *memfs) { return memfs->getStatus(); }

FuseStatus umount(FuseMemfs *memfs) { return memfs->stop(); }
